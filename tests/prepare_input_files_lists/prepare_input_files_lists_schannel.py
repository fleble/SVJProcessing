...
def __prepare_input_files_lists(
    ...
    bash_command = f"python {os.environ['SVJ_PROCESSING_ROOT']}/prepare_input_files_lists/list_dataset_files.py -d {dataset_name} -y {year} -c {dataset_config} -o {dataset_directory} -nano"
    helper.test_command(bash_command)

    bash_command = f"python {os.environ['SVJ_PROCESSING_ROOT']}/prepare_input_files_lists/compute_unweighted_selection_efficiency.py -d {dataset_name} -y {year} -p {analysis_config} -s {selection_name} -i {dataset_directory} -o {dataset_directory} -e {executor} -n {n_workers} -c {chunk_size} -nano"
    helper.test_command(bash_command)
...
def test_execution():
    ...
    years = ["2018"]
    dataset_name = "mMed-2000GeV_mDark-8GeV_rinv-0.3"
    dataset_config = "dataset_configs.tests_schannel"
    analysis_config = "analysis_configs.s_channel_leptons_mc_pre_selection"
    selection_name = "s_channel_pre_selection"
