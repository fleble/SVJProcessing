...
def get_model(data_config, **kwargs):
  conv_params = data_config.conv_params
  fc_params = data_config.fc_params
  use_fusion = True
  features_dims = len(data_config.input_dicts['features'])
  num_classes = len(data_config.label_value)
  model = SVJTagger(features_dims,
                    num_classes,
                    conv_params,
                    fc_params,
...
def knn(x, k: int):
  ...
  idx = pairwise_distance.topk(
      k=k + 1, dim=-1)[1][:, :, 1:]  # (batch_size, num_points, k)
  return idx
...
def get_graph_feature_v1(x, k: int, idx):
  ...
  idx_base = torch.arange(0, batch_size, device=x.device).view(-1, 1,
                                                               1) * num_points
  idx = idx + idx_base
  idx = idx.view(-1)
  ...
  fts = fts[idx, :].view(
      ...
      num_dims)  # neighbors: -> (batch_size*num_points*k, num_dims) -> ...
  fts = fts.permute(0, 3, 1,
                    2).contiguous()  # (batch_size, num_dims, num_points, k)
  x = x.view(batch_size, num_dims, num_points, 1).repeat(1, 1, 1, k)
  fts = torch.cat((x, fts - x),
                  dim=1)  # ->(batch_size, 2*num_dims, num_points, k)
  return fts
...
def get_graph_feature_v2(x, k: int, idx):
  ...
  fts = fts[:, idx].view(
      num_dims, batch_size, num_points,
      k)  # neighbors: -> (num_dims, batch_size*num_points*k) -> ...
...
class EdgeConvBlock(nn.Module):
  ...
  def __init__(self,
    ...
    for i in range(self.num_layers):
      self.convs.append(
          nn.Conv2d(2 * in_feat if i == 0 else out_feats[i - 1],
                    out_feats[i],
                    kernel_size=1,
  ...
  def forward(self, points, features):
    ...
    x = get_graph_feature_v2(
        features, self.k,
        topk_indices) if self.cpu_mode else get_graph_feature_v1(
            features, self.k, topk_indices)
...
class SVJGNN(nn.Module):
  def __init__(self,
               ...
               **kwargs):
    super(SVJGNN, self).__init__(**kwargs)
    ...
    if self.use_fusion:
      ...
      self.fusion_block = nn.Sequential(
          ...
          nn.BatchNorm1d(out_chn), nn.ReLU())
    ...
    for idx, layer_param in enumerate(fc_params):
      ...
      if self.for_segmentation:
        fcs.append(
            nn.Sequential(nn.Conv1d(in_chn, channels, kernel_size=1, bias=False),
                          nn.BatchNorm1d(channels), nn.ReLU(),
                          nn.Dropout(drop_rate)))
      else:
        fcs.append(
            nn.Sequential(nn.Linear(in_chn, channels), nn.ReLU(),
                          nn.Dropout(drop_rate)))
    if self.for_segmentation:
...
class SVJTagger(nn.Module):
  def __init__(self,
               ...
               for_inference=False,
               **kwargs):
    super(SVJTagger, self).__init__(**kwargs)
    self.pf_input_dropout = nn.Dropout(input_dropout) if input_dropout else None
    self.pf_conv = FeatureConv(features_dims, 32)
    self.pn = SVJGNN(input_dims=32,
