...
class DataConfig(object):
  r"""Data loading configuration.
    """
  def __init__(self, in_yaml, print_info=True):
    ...
    self.test_time_selection = opts['test_time_selection'] if opts[
        'test_time_selection'] else self.selection
    self.var_funcs = opts['new_variables']
    # preprocessing config
    self.preprocess = opts['preprocess']
    self._auto_standardization = opts['preprocess']['method'].lower().startswith(
        'auto')
    self._missing_standardization_info = False
    self.preprocess_params = opts['preprocess']['params'] if opts['preprocess'][
        'params'] is not None else {}
    # inputs
    ...
    for k, o in opts['inputs'].items():
      ...
      for v in o['vars']:
        ...
        if opts['preprocess']['params'] is None:
          ...
          if v[0] in self.preprocess_params and params != self.preprocess_params[
              v[0]]:
            raise RuntimeError(
    ...
    if self.label_type == 'simple':
      ...
      self.var_funcs['_label_'] = 'np.stack([%s], axis=1).argmax(1)' % (','.join(
          self.label_value))
    else:
      self.label_names = tuple(self.label_value.keys())
      self.var_funcs.update(self.label_value)
    # modelStructure
    num_of_k_nearest = opts['modelStructure']['num_of_k_nearest']
    num_of_edgeConv_convLayers = opts['modelStructure']['num_of_edgeConv_convLayers']
    num_of_edgeConv_dim = opts['modelStructure']['num_of_edgeConv_dim']
    conv_params = []
    ...
    num_of_fc_layers = opts['modelStructure']['num_of_fc_layers']
    num_of_fc_nodes = opts['modelStructure']['num_of_fc_nodes']
    fc_dropout = opts['modelStructure']['fc_dropout']
    fc_params = []
    ...
    self.z_variables = self.observer_names if len(
        self.observer_names) > 0 else self.monitor_variables
    ...
    if print_info:
      print('preprocess config: %s' % str(self.preprocess))
      print('selection: %s' % str(self.selection))
      print('test_time_selection: %s' % str(self.test_time_selection))
      print('var_funcs:\n - %s' %
            '\n - '.join(str(it) for it in self.var_funcs.items()))
      print('input_names: %s' % str(self.input_names))
      print('input_dicts:\n - %s' %
      ...
      print('label_names: %s' % str(self.label_names))
      print('observer_names: %s' % str(self.observer_names))
      print('monitor_variables: %s' % str(self.monitor_variables))
      if opts['weights'] is not None:
        if self.use_precomputed_weights:
          print('weight: %s' % self.var_funcs[self.weight_name])
        else:
          for k in [
              'reweight_method', 'reweight_branches', 'reweight_bins',
    ...
    self.load_branches = (aux_branches | self.keep_branches) - set(
        self.var_funcs.keys()) - {self.weight_name, }
    if print_info:
