...
def get_collection(events, collection_name, variables=[], zero_variables=[]):
    ...
    collection = ak.with_field(collection, vecUtl.rapidity(collection), where="rapidity")     
    collection = ak.with_field(collection, vecUtl.momentum(collection), where="momentum")
    return collection
...
def get_met(events, collection_name="MET", additional_variables=[]):
    variables = ["pt", "phi"] + additional_variables
    zero_variables = ["eta", "mass"]
    met = get_collection(events, collection_name, variables, zero_variables)
...
def get_jets(events, collection_name, additional_variables=[]):
    ...
    #"subJetIdx1", "subJetIdx2","tau1", "tau2", "tau3", "tau4", "hvCategory", "genJetIdx"
...
def get_subjets(events, collection_name, additional_variables=[]):
    variables = ["pt", "eta", "phi", "mass"] + additional_variables
    subjets = get_collection(events, collection_name, variables)
    return subjets
...
def get_leptons(events, collection_name, additional_variables=[]):
    variables = ["pt", "eta", "phi", "mass", "charge", "pdgId"] + additional_variables
    leptons = get_collection(events, collection_name, variables)
    return leptons


def get_photons(events, collection_name, additional_variables=[]):
    variables = ["pt", "eta", "phi"] + additional_variables
    zero_variables = ["mass"]
    photons = get_collection(events, collection_name, variables, zero_variables)
    return photons


def get_tracks(events, collection_name, additional_variables=[]):
    variables = ["pt", "eta", "phi", "pdgId", "d0", "dz", "charge"] + additional_variables
    zero_variables = ["mass"]
    tracks = get_collection(events, collection_name, variables, zero_variables)
    return tracks